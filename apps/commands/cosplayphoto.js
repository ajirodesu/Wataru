...
export const config = {
  ...
  description: "get a random cosplay photo",
  usage: [""],
  cooldown: 0,
  access: "anyone",
  category: "anime"
...
export const onCommand = async function({ message, bot, chatId, log }) {
  try {
    ...
    await bot.sendPhoto(chatId, link, { caption: `Here's a random cosplay photo!` });

  } catch (error) {
    log.error("Error fetching cosplay data:", error);
